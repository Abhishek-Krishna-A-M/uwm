...
#define RUN         "bash", "-c", "cmd=$(compgen -c | sort -u | ulaunch -d -p ' Run: ') && [ -n \"$cmd\" ] && eval \"$cmd\"", NULL
...
#define FINDFILE    "sh", "-c", "f=$(cd ~ && fd --type f --hidden --exclude .git --exclude .cache --exclude .local --exclude node_modules --exclude .npm --exclude .cargo --exclude .rustup --exclude .gradle --exclude .m2 --exclude .vscode | ulaunch -d -p '󰈞 Find File: ') && [ -n \"$f\" ] && foot -e nvim \"$HOME/$f\"", NULL
