...
static void focus_toplevel(struct uwm_toplevel *toplevel) {
...
static bool handle_keybinding(struct uwm_server *server, xkb_keysym_t sym) {
	...
	case XKB_KEY_F1:
		...
		struct uwm_toplevel *next_toplevel =
...
static void server_new_keyboard(struct uwm_server *server,
...
static void server_new_pointer(struct uwm_server *server,
		struct wlr_input_device *device) {
...
static void seat_request_cursor(struct wl_listener *listener, void *data) {
	struct uwm_server *server = wl_container_of(
...
static struct uwm_toplevel *desktop_toplevel_at(
...
static void reset_cursor_mode(struct uwm_server *server) {
...
static void begin_interactive(struct uwm_toplevel *toplevel,
		enum uwm_cursor_mode mode, uint32_t edges) {
